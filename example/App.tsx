...
import { Davina, DavinaProvider } from '@bornworks/davina';

// 1) Init as early as possible — before any request fires.
//    In a real app, put this at the top of index.js.
Davina.init({
  showNotification: true, // floating bubble on new calls; Davina.setNotification(false) to silence
  maxCallsCount: 500,
...
function Demo() {
  ...
  const doAxiosPost = () =>
    axios
      .post(`${BASE}/posts`, { title: 'hello', body: 'from Davina example' })
      .catch(() => {});
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.title}>Davina example</Text>
      <View style={styles.buttons}>
        ...
        <Button title="Open inspector" onPress={() => Davina.open()} />
        <Button title="Mute notifications" onPress={() => Davina.setNotification(false)} />
        <Button title="Unmute notifications" onPress={() => Davina.setNotification(true)} />
        <Button title="Clear captured calls" onPress={() => Davina.clear()} />
      </View>
